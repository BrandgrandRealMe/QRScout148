...
export function PreviewText(props: PreviewTextProps) {
  ...
  return (
    <div className="flex flex-col items-center gap-2 shadow-md bg-gray-1100 m-2 p-2 rounded-md">
      <div className="text-justify p-2 rounded bg-gray-1100 ">
        <p className=" font-mono text-wrap break-all text-black-200 ">
          {chunks.map((c, i) => (
            <>
              <span key={i + c}>{c}</span>

              <span key={i + c + 'tab'} className="text-black-500">
                {i !== chunks.length - 1 ? '|' : ' ↵'}
              </span>
            </>
          ))}
